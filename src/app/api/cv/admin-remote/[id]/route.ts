...
// Safari-safe CV open for /admin/remote-talent (Option C). Mirrors
...
export async function GET(
  ...
  if (profileErr || !profileRow) {
    return htmlError(
      404,
      "CV not found",
      "We couldn't find this remote talent profile.",
    );
  }
  const cvPath =
  ...
  if (!cvPath) {
    return htmlError(
      404,
      "CV not found",
      "This profile doesn't have a CV file attached.",
    );
  }
