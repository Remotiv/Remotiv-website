...
// Safari-safe CV open for /admin/client-batches batch-detail (Option C). Mirrors
// /api/cv/admin-application/[id] exactly; only the lookup table + source_table
// differ. Admin-only — requireAdmin replaces the dual client/admin ACL that
// client/actions.ts's getClientCvSignedUrl uses for the shared client portal.
...
export async function GET(
  ...
  const { data: candRow, error: candErr } = await service
  ...
  if (candErr || !candRow) {
    return htmlError(
      404,
      "CV not found",
      "We couldn't find this batch candidate.",
    );
  }
  const cvPath =
    (candRow.cv_path as string | null) ??
    deriveCvPathFromUrl(candRow.cv_url as string | null);
  if (!cvPath) {
    return htmlError(
      404,
      "CV not found",
      "This candidate doesn't have a CV file attached.",
    );
  }
