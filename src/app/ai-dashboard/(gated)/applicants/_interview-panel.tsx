...
export function InterviewPanel({
  ...
  const load = useCallback(async () => {
    try {
      setSession(await fetchInterviewSession(applicationId));
    } catch {
      setSession(null);
    } finally {
      setLoading(false);
  ...
  const state = session ? (STATE[session.status] ?? STATE.invited) : null;
  const StateIcon = state?.icon ?? Video;
  const canResend =
      ...
      {session && state && (
        <div className="rounded-xl border border-[var(--ai-line)] bg-[var(--ai-surface)] px-3.5 py-3">
          <div className="flex items-center gap-2.5">
            <span
              className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${state.cls}`}
            >
