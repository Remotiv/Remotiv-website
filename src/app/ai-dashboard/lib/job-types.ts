...
/*
 * The weighting vocabulary now lives in src/lib/weights.ts — a module with no
 * imports, so `node --test` can load it and weights.test.ts can assert the
 * round trip. Re-exported here so every existing importer is unchanged.
 *
 * It moved because three defects lived in it undetected: "Most" stored as 5,
 * a default of 1 that means Less, and a CV fallback of 3 that is not a stop.
 * All three came from restating the stops' range and default elsewhere.
 */
export {
  CV_WEIGHT_DEFAULT,
  CV_WEIGHT_MAX,
