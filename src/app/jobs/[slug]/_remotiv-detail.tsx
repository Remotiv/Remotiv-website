...
import { isRemotivOwned, type Job } from "@/lib/jobs";
 ...
 *
 * A job with no company_id belongs to Remotiv and keeps rendering exactly as it
 * did before the white-label rebuild: same hero, perks marquee, flip cards,
 * hiring timeline and boarding-pass ticket. The JSX below was moved here
 * verbatim; only the helpers it calls now arrive by import rather than being
 * declared above it, which changes nothing it renders.
 *
 * The white-label design retires the 1.85fr/1fr hero split and the .hcard
 * role-summary card BY NAME — but that instruction governs the COMPANY page.
 * This one is the design those structures belong to, and it keeps them.
 */
...
const TIMELINE = [
  {
    ...
    sub: "One form, no cover letter required.",
    dot: "#D9F972",
    ink: "#2c3d05",
  },
  {
  ...
  },
  {
    ...
    sub: "Meet the hiring team — usually next day.",
    dot: "#9886FE",
    ink: "#ffffff",
  },
  {
    n: "4",
    title: "Offer within 2 weeks",
    sub: "From application to signed — two weeks.",
    dot: "#ffffff",
    ink: "#241f38",
  },
];
...
function SectionBar({
  ...
  return (
    <div className="mb-3 flex items-center gap-2.5">
      ...
      <h2
        className={`font-heading font-bold text-[#241F38] ${size === "lg" ? "text-[17px]" : "text-[15px]"}`}
      >
        {label}
      </h2>
    </div>
...
export function RemotivJobDetail({
  ...
  // Was `params.slug` before the split. getJobBySlug found this row BY that
  // slug, so job.slug is the same string and the URL is unchanged.
          ...
          <div>
            ...
            <div className="mt-5 inline-flex flex-wrap items-center gap-3 rounded-[15px] border border-white/[0.18] bg-white/[0.12] px-4 py-2.5 backdrop-blur">
              ...
              {job.company_logo_url ? (
                // biome-ignore lint/performance/noImgElement: a Supabase public URL on a per-tenant bucket path — next/image would need every company's host allow-listed. Unchanged from before the split; only the linter named in the suppression is now the one this project actually runs.
                <img
            ...
            <div className="flex flex-col items-end gap-2.5">
              {CITY_CHIPS.map((c) => (
                ...
                >
                  ...
                  <span className="font-heading text-[0.8rem] font-bold text-[#241f38]">
                    {c.label}
                  </span>
                </div>
              ))}
                ...
                <ul className="mb-5 flex flex-col gap-2.5">
                  {responsibilities.map((line) => (
                    <li key={line} className="flex gap-2.5 text-sm leading-[1.55] text-[#4b4660]">
                      <IconCircleCheckFilled
                        size={19}
                        className="shrink-0 text-remotiv-mint"
                        style={{ color: "#49D7A7" }}
                      />
                      <span>{line}</span>
                    </li>
            ...
            <div className="relative">
              ...
              {TIMELINE.map((s) => (
                <div key={s.n} className="jp-tstep">
                  <span
                    className="jp-tdot font-heading"
                    style={{ background: s.dot, color: s.ink }}
                  >
                    {s.n}
                  </span>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="jp-ticket">
            <div className="p-[22px]">
              ...
              <p className="font-heading text-[27px] font-extrabold tracking-[-0.03em] text-[#241f38]">
                {salary}
              </p>
              ...
              <div>
                {facts.map(({ Icon, label, value }) => (
                  <div
                    key={label}
                    className="flex items-center gap-3 border-b border-[#efeae3] py-3 last:border-b-0"
                  >
