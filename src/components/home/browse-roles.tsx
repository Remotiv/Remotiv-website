    ...
    <section className="bg-remotiv-purple-light px-6 py-12 sm:px-14 md:py-16 lg:py-20">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {ROLES.map((role) => {
            ...
            return (
              <Link
                key={role.name}
                href="/browse-talent"
                className="group flex min-h-[200px] sm:min-h-[180px] flex-col items-center justify-center gap-5 rounded-2xl border border-[#252525] bg-white px-5 py-8 text-center no-underline transition-[border-color,background,transform] duration-200 hover:-translate-y-0.5 hover:border-remotiv-purple-light hover:bg-[#f5f5f5]"
              >
                <Icon className="size-7 fill-[#111] text-[#111] opacity-[0.85]" strokeWidth={2} />
                <span className="font-sans text-[0.85rem] font-semibold leading-[1.4] text-[#111] sm:text-[0.92rem]">
                  {role.name}
                </span>
              </Link>
            );
          ...
          <div className="flex min-h-[200px] sm:min-h-[180px] flex-col items-center justify-center gap-4 rounded-2xl border border-[#252525] bg-white px-5 py-8 text-center">
            <p className="font-sans text-base font-semibold leading-[1.4] text-[#111]">
              Looking for another role?
            </p>
            <Link
              href="/contact"
              className="inline-flex items-center gap-1.5 rounded-full bg-[#111] px-[22px] py-3 font-sans text-[0.82rem] font-bold tracking-[0.04em] text-white no-underline transition-colors duration-200 hover:bg-[#333]"
            >
              Place request →
            </Link>
          </div>
